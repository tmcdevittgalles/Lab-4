...
There are four built-in calculations in R that are commonly used to work with the normal distribution. You will be using all 4 of these today.
  ...
  3. `pnorm(q, mean, sd)` gives  the area under the normal curve *to the left* of value *q*. To get the area under the qurve to the *right* of *q*, you can specify `pnorm(q, mean, sd, lower.tail=FALSE)`. The function `pnorm()` will generate the cumulative distribution function if *q* is a sequence of numbers rather than a single number.
...
The normal distribution plot shows the probability density of each value of X for the standard normal distribution ($\mu$ = 0, $\sigma$ = 1). We could vary the other arguments in the `dnorm()` function to alter the shape of the curve. Some notes on the plot : by including `type = "l` in the plotting argument I am plotting a smoothed line curve, rather than a bunch of different (x, y) points. 
...
\textcolor{red}{\textbf{Question 5.} For a normal distribution with $\mu = 50$ and $\sigma = 0.5$, calculate the X value at which only $5$ percent of the area is to the left (less than or equal to X). Confirm this using $pnorm()$. Take a look at the four functions I've described at the beginning of the lab and pick the appropriate function.}
